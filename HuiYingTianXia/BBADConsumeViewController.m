...
@interface BBADConsumeViewController ()<AC_POSMangerDelegate,MF_POSMangerDelegate,MPosDelegate>
...
@implementation BBADConsumeViewController
...
- (void)waitingForCardSwipe:(BOOL)status
{
    if(status == SUCCESS) {
        ...
        BBADSwingCardViewController *swingCardVC = [[BBADSwingCardViewController alloc] init];
        swingCardVC.tadeType = self.tadeType;
