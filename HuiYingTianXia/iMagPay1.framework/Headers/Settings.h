...
-(NSString*)blePinOpen:(NSString *)pan;

-(NSString*)blePinClose;
...
-(NSString *)parseApduData:(NSString *)data;
#pragma mark 国内第三方支付平台接口
/**获取加密PIN*/
-(NSString*)getEncryptedPIN:(PAY_COMPANY)company :(NSString*)pan :(NSString*)pin;
/**下载工作密钥*/
-(BOOL)writeSignIn:(PAY_COMPANY)company :(NSString*)data;

/**深*福建 工作密钥**/
-(BOOL)writeSignIn:(NSString*)data;

//ON/OFF ic detect function
-(BOOL)writeICDetect:(BOOL)value;
/**
 *下载主密钥
 */
-(BOOL)writeTMK:(PAY_COMPANY)company :(NSString *)key;
/**
 *获取加密MAC
 */
-(NSString *)getMacBlock:(PAY_COMPANY)company :(NSString *)data;

-(NSString *)getMacBlock:(PAY_COMPANY)company :(NSString *)vector :(NSString *)data;
/**磁道加密密钥*/
-(BOOL)writeTDK:(PAY_COMPANY)company :(NSString *)key;

/**查询是否有IC卡*/
//返回00表示有卡,ff表示无卡
-(NSString *)writeDetectCard;

//修改蓝牙读头名称
-(BOOL)writeBLEName:(NSString *)name;

-(NSString *)getEncryptedData:(PAY_COMPANY)company :(NSString *)data;
//单例
+(id)sharedInstance;

//2015.12.30新增
-(BOOL)setTrackEncryptedMode:(TRACK_ENCYPTED_MODE)mode;

//2015.12.11 带按键LED一体MPOS接口
//开始EMV流程,传入日期,时间,金额
//日期,时间格式:YYMMDD HHMMSS
-(NSString *)startEmv:(NSString *)date :(NSString *)time :(int)amount;
//传入金额,日期SDK内部取当前时间
-(NSString *)startEmv:(int)amount;
//是否有卡
-(BOOL)isCardHere;
//获取TLV数据
-(NSString *)getEmvTLVDataWithTag:(NSString *)tag;
@end
