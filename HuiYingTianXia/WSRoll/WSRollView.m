...
-(void)addRollImageAndTimer{
    ...
    //图片
    if (_rollImage !=nil && _rollImage.size.width/_rollImage.size.height<self.frame.size.width/self.frame.size.height && ABS((self.frame.size.height/_rollImage.size.height) *_rollImage.size.width) - self.frame.size.width > self.frame.size.width/4) {
