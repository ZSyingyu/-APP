...
@interface vcom:NSObject
{
    ...
    char* retdata;
    int retdatalen;
    //    id<CSwiperStateChangedListener> _eventListener;
    int itronLog;
    CommunicationMode cMode;
    
}
@property(retain,nonatomic) NSTimer *itTimer;
@property(nonatomic) char* retdata;
@property(nonatomic) int retdatalen;
@property(nonatomic,assign) id<CSwiperStateChangedListener> eventListener;
/******************************************************/
//得到音频对象的全局变量
+(vcom*)getInstance;
//打开音频收发数据功能
...
//钱箱控制命令
- (void)cashboxControl:(int)time;

- (void)updateStatus1:(NSTimer*)timer;
