...
@interface XNBTADConsumeViewController ()<AC_POSMangerDelegate,MF_POSMangerDelegate,MPosDelegate>
...
@implementation XNBTADConsumeViewController
...
- (void)waitingForCardSwipe:(BOOL)status
{
    if(status == SUCCESS) {
        ...
        XNBTADSwingCardViewController *swingCardVC = [[XNBTADSwingCardViewController alloc] init];
        swingCardVC.tadeType = self.tadeType;
        swingCardVC.rate = self.rate;
        swingCardVC.tadeAmount = self.labNumber.text;
        NSLog(@"amount:%@",swingCardVC.tadeAmount);
